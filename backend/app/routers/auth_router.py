...
async def login(payload: LoginRequest, request: Request, response: Response, svc: AuthService = Depends(get_auth_service)):
    # Accept either email or nickname in payload.identifier
    user = await svc.login(request, response, payload.identifier, payload.password)  # type: ignore[attr-defined]
    return {"user": user}
